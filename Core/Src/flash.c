...
{
	if ((Address < FLASH_BASE) || (Address > FLASH_SIZE + FLASH_BASE))  return FLASH_SECTOR_TOTAL;
...
int16_t Flash_Program_Data (uint32_t StartAddress, uint32_t *pData, uint32_t noOfData)
{
	...
	{
		if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, StartAddress, (uint32_t ) &pData[acktNoOfData]) == HAL_OK)
		{
